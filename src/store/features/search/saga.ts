...
function* create(action: PayloadAction<CreateSearch>) {
    ...
    try {
        ...
        const { previewImage }: SearchState = yield select(
...
function* update(action: PayloadAction<{ id: number; update: UpdateSearch }>) {
    ...
    try {
        const res: ApiResponse<Search | ErrorResInterface> =
            yield apiCall.update(action.payload.id, action.payload.update);
...
function* destroy(action: PayloadAction<number>) {
    ...
    try {
        const res: ApiResponse<Search | ErrorResInterface> =
            yield apiCall.delete(action.payload);
        yield putEndLoading();
        if (!res.isSuccess) {
...
function* uploadImage(
    ...
    yield putStartLoading();
    try {
        const fileBlob: Blob | null = yield call(
            toBlob,
            action.payload.file.url,
        );
        if (fileBlob == null) {
